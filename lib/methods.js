...
import { ProblemTemplates } from './problemtemplates.js';
...
export const insertCourse = (attributes) => {
  ...
  const { name, description, startAt, endAt, coursetype } = attributes;
  const course = {
    name: name,
...
export const addSingleProblemTemplateToCourse = (courseId, templateId) => {
	return addMultipleProblemTemplateToCourse(courseId, [templateId]);
};

export const addMultipleProblemTemplateToCourse = (courseId, templateIds) => {
	return Courses.update({_id: courseId}, {$push: {problemtemplates: { $each: templateIds}}});
};
...
// Problem Sets
export const findAllProblemsets = () => {
  return ProblemSets.find().fetch();
};

export const findRandomProblemsets = () => {
  return ProblemSets.find().fetch();
};

// Knowledge Points
export const findAllKnowledgePoints = () => {
  return KnowledgePoints.find().fetch();
};

export const findKnowledgePointsByIds = (idArray) => {
  return KnowledgePoints.find({_id: { $in: idArray}}).fetch();
};

// Course Type
export const findCourseTypeById = (id) => {
  return CourseType.findOne({ _id: id});
};

export const findAllCourseTypes = () => CourseType.find().fetch();

// Problem Template
export const insertProblemTemplate = (attributes) => {
	const { name, description, configs } = attributes;
	...
	const templateId = ProblemTemplates.insert(problemTemplate);
	return { _id: templateId };
};

export const findProblemTemplatesByIds = (idArray) => {
  return ProblemTemplates.find({_id: { $in: idArray}}).fetch();
};